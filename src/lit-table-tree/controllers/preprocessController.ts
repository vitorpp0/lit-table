...
export type AggOperation = 'max' | 'min' | 'count' | 'mean' | 'sum' | 'first';
...
export class ProcessController implements ReactiveController {
  ...
  groupByKeys(elements:DataRows, keys:GroupBy) {
      return elements.reduce((result, element) => {
        ...
        keys.forEach((key, index) => {
          ...
          if(!group[keyValue]) group[keyValue] = index === keys.length - 1 ? [] : {};
  ...
  apply(key:string, aggFunc:AggOperation, rows:DataRows){
      switch(aggFunc){
        ...
        case "sum":
          return this.sum(rows, key);
        case "first":
          return this.first(rows, key);
        default:
  ...
  sum(arr:DataRows, key:string) {
    return arr.reduce((sum, item) => sum + item[key], 0);
  }

  first(arr:DataRows, key:string) {
  ...
  // float precision
  setPrecision(precision=2){
    this.table = this.table.map(row=>{
      for(const key in row){
        if(typeof row[key]==="number"){
          if(row[key]%1!==0){
            row[key] = row[key].toFixed(precision); 
          }
        }
      }
      return row
